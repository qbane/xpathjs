...
// converted to ES module and adjusted for TS type inferences; from
 ...
 */
const nodeChildren = function(node)
  {
    var nodes = [],
 ...
 * @return {Array} List of nodes in document order.
 */
const nodeDescendant = function(node)
  {
    var nodes,
      i,
      nodes2 = []
 ...
 * @return {Node}
 */
const nodeParent = function(node)
  {
    /**
 ...
 */
const nodeAncestor = function(node)
  {
    var parent,
      nodes = []
    ;
 ...
 * @return {Array} List of nodes in document order
 */
const nodeFollowingSibling = function(node)
  {
 ...
 * @return {Array} List of nodes in document order
 */
const nodeFollowing = function(node)
  {
    var nodes = [],
      parents,
      i,
 ...
 * @return {Array} List of nodes in reverse document order
 */
const nodePreceding = function(node)
  {
    var nodes = [],
      parents,
      i,
 ...
 * @return {Document}
 */
const nodeOwnerDocument = function(node)
  {
    switch(node.nodeType)
    {
      case 9: // document
 ...
 */
const nodeGetAttributes = function(node)
  {
    var nodes = [],
      i
    ;
 ...
 * @return {Array} List of attribute nodes in document order
 */
const nodeAttribute = function(node)
  {
 ...
 */
const nodeNamespace = function(node, nsNodes)
  {
    var nodes = (nsNodes || []),
      allAttributeNodes,
      i,
...
const insertNamespaceIfNotDeclared = function(namespaces, prefix, ns, parent)
  {
    var i, namespace;
...
const nodeIdAttribute = function(node, attribute)
  {
    var i,
      j,
      attributes,
...
const nodeAttributeSearch = function(startNode, stopAfterFirstMatch, fn)
  {
    var i,
      j,
      elements,
...
const nodeExpandedName = function(node)
  {
    var name,
      namespaces,
      i,
...
const nodeStringValue = function(node)
  {
    var i,
      nodeset,
      value = ''
...
const createError = function(code, name, message)
  {
 ...
 * @return {Number}
 */
const arrayIndexOf = function(needle, haystack)
  {
 ...
 */
const compareOperator = function(left, right, operator, compareFunction)
  {
    var i,
      j,
      leftValues,
...
const getComparableNode = function(node)
  {
    switch(node.nodeType)
    {
      case 2: // attribute
...
const compareDocumentPosition = function(a, b)
  {
    var result, nodes, i;
 ...
 */
const compareDocumentPositionNoNamespace = function(a, b)
  {
    var a2,
      b2,
      result,
...
const nodeSupported = function(contextNode)
  {
    if (!contextNode) {
...
const createNamespaceNode = function(prefix, ns, parent)
  {
    var i, namespaceNode;
...
const BaseType = function(value, type, supports)
{
  this.value = value;
  this.type = type;
...
const BooleanType = function(value)
{
...
const NodeSetType = function(value, documentOrder)
{
...
const StringType = function(value)
{
...
const NumberType = function(value)
{
 ...
 *
 */
const XPathException = function(code, message)
{
  var err;
 ...
 */
const XPathEvaluator = function(options)
{
...
XPathEvaluator.prototype = {
  opts: {
    ...
    //! whether to include additional functions and axes described in IV docs
    withNonstandardUtilities: true,
 ...
 */
const XPathExpression = function(parsedExpression, namespaceMapping, options) {
 ...
 */
const Context = function(node, position, size, vars, functions, namespaceMap, options)
{
  this.node = node;
  this.pos = position;
  this.size = size;
  ...
  if (this.opts.withNonstandardUtilities) {
    Object.assign(this.fns[''], nonstandardFunctions)
  }
}
 ...
 */
const XPathNSResolver = function(nodeResolver)
{
...
const expressions = {
  ...
  step: function(axis, nodeTest)
  {
    ...
    switch(axis)
    {
      ...
      default:
        //! handle additional axes
        if (this.opts.withNonstandardUtilities) {
          if (axis === 'prev-sibling') {
            // like "preceding-sibling::*[1]/self"
            nodeset = new NodeSetType(nodeXSibling(this.node, 'previousSibling', true), 'reverse-document-order');
          } else if (axis === 'next-sibling') {
            // like "following-sibling::*[1]/self"
            nodeset = new NodeSetType(nodeXSibling(this.node, 'nextSibling', true), 'document-order');
          }
        }
...
//! nonstandard functions that are included when the option "withNonstandardUtilities" is set
const nonstandardFunctions = {
  'has-class': {
    fn: function(str) {
      const normalizeSpace = functions['']['normalize-space'].fn
      const className = new StringType(this.node.className)

      return new BooleanType(` ${normalizeSpace(className).toString()} `.indexOf(` ${str} `) != -1)
    },
    args: [{t: 'string'}],
    ret: 'boolean',
  },
  ...
  'ends-with': {
    ...
    {
      needle = needle.toString()
      return new BooleanType(haystack.toString().slice(haystack.length - needle.length) == needle);
    },
 ...
 * @return {Object}
 */
const evaluateExpressionTree = function(context, tree)
{
 ...
 */
const XPathResult = function(context, type, value)
{
  switch(type)
  {
 ...
 */
const XPathNamespace = function(prefix, namespaceURI, ownerElement)
{
 ...
 * that are in use.
 *
 * @return List of DOM Level 3 XPath window and document objects
 *         that are currently in use.
 */
export const getCurrentDomLevel3XPathBindings = function() {
  return {
    'window': {
 ...
 * the XPathJS module.
 *
 * @return List of DOM Level 3 XPath objects implemented by
 *         the XPathJS module.
 */
export const createDomLevel3XPathBindings = function(options) {
 ...
 */
export const bindDomLevel3XPath = function(bindings) {
...
const module = {
  ...
  XPathResult,
  XPathNamespace,
  getCurrentDomLevel3XPathBindings,
